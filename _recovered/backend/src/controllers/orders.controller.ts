...
export async function listOrdersController(req: AuthedRequest, res: Response): Promise<void> {
  const status = typeof req.query.status === 'string' ? req.query.status : undefined;
  const channel = typeof req.query.channel === 'string' ? req.query.channel : undefined;
  res.json({ data: await listOrders({ status, channel }) });
}

export async function getOrderController(req: AuthedRequest, res: Response): Promise<void> {
...
export function createOrderController(bus: EventBus) {
  return async (req: AuthedRequest, res: Response): Promise<void> => {
    ...
    const created = await createOrder(body.data);
...
export function patchOrderController(bus: EventBus) {
  return async (req: AuthedRequest, res: Response): Promise<void> => {
    ...
    const updated = await updateOrderStatus(req.params.id, body.data.status);
    if (!updated) {
      res.status(404).json({ message: 'Order not found' });
      return;
    }
...
export function advanceOrderController(bus: EventBus) {
  return async (req: AuthedRequest, res: Response): Promise<void> => {
    ...
    const updated = await updateOrderStatus(current.id, ORDER_STATUS_FLOW[index + 1]);
    if (!updated) {
      res.status(404).json({ message: 'Order not found' });
      return;
    }
