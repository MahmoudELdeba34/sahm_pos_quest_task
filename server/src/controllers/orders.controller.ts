...
export function deleteOrderController(bus: EventBus) {
  return (req: AuthedRequest, res: Response): void => {
    const existing = getOrder(paramId(req.params.id));
    if (!existing) {
      res.status(404).json({ message: 'Order not found' });
      return;
    }
    deleteOrder(paramId(req.params.id));
    const at = new Date().toISOString();
    bus.publish({ type: 'order.deleted', payload: { id: paramId(req.params.id) }, at });
    res.status(204).send();
  };
}
...
export function patchOrderController(bus: EventBus) {
  return (req: AuthedRequest, res: Response): void => {
    ...
    const updated = updateOrderStatus(paramId(req.params.id), body.data.status);
    if (!updated) {
      res.status(404).json({ message: 'Order not found' });
      return;
    }
...
export function advanceOrderController(bus: EventBus) {
  return (req: AuthedRequest, res: Response): void => {
    const current = getOrder(paramId(req.params.id));
    if (!current) {
      res.status(404).json({ message: 'Order not found' });
      return;
    }
