...
import type { KitchenLevel, OrderStatus } from '../types/domain.js';
...
export function startSimulation(bus: EventBus): () => void {
  let kitchenPercent = getKitchen().overallPercent || 55;
  ...
  const orderTimer = setInterval(() => {
    ...
    if (roll < 0.85) {
      const active = listOrders().filter((o) => o.status !== 'completed' && o.status !== 'cancelled');
      if (!active.length) return;
      const order = active[Math.floor(Math.random() * active.length)];
      const index = FLOW.indexOf(order.status);
      if (index < 0 || index >= FLOW.length - 1) return;
      const updated = updateOrderStatus(order.id, FLOW[index + 1]);
      if (!updated) return;
      bus.publish({ type: 'order.status', payload: { orderId: updated.id, status: updated.status, source: 'websocket' }, at });
  ...
  const kitchenTimer = setInterval(() => {
    ...
    const updatedOrders = markOrdersDelayed(delayedOrderIds);
