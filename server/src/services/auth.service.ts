...
function issueSession(user: AuthUser): { accessToken: string; refreshToken: string; user: AuthUser } {
...
export function refresh(refreshToken: string): { accessToken: string; user: AuthUser } | null {
...
export async function createUserAccount(input: {
  ...
  name: string;
  role: Role;
}): Promise<AuthUser | { error: 'email_taken' }> {
  const email = input.email.toLowerCase().trim();
  if (findUserByEmail(email)) {
    return { error: 'email_taken' };
  }
